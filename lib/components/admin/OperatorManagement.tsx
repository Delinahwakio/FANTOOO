...
import { Operator } from '@/lib/types/user';
import { cn } from '@/lib/utils/cn';

export interface OperatorManagementProps {
  className?: string;
}

/**
 * OperatorManagement Component
 * 
 ...
 * ```
 */
export const OperatorManagement: React.FC<OperatorManagementProps> = ({ className }) => {
  ...
  const handleSuspendOperator = async (operatorId: string, reason: string, duration: number) => {
    try {
      // TODO: Replace with actual API call
      // await fetch(`/api/admin/operators/${operatorId}/suspend`, {
      //   method: 'POST',
  ...
  const handleReactivateOperator = async (operatorId: string) => {
    try {
      // TODO: Replace with actual API call
      // await fetch(`/api/admin/operators/${operatorId}/reactivate`, {
      //   method: 'POST',
      // });
                ...
                <div className={cn('p-3 rounded-xl', getQualityScoreBg(operator.quality_score))}>
                  ...
                  <div className="mt-2 h-2 bg-white/50 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: `${operator.quality_score}%` }}
                    />
                  </div>
                </div>
              ...
              {selectedOperator.is_suspended ? (
                ...
                <GlassButton
                  ...
                  onClick={() => {
                    ...
                    if (reason) {
                      handleSuspendOperator(selectedOperator.id, reason, 7);
                    }
                  }}
                >
                  Suspend Operator
      ...
      >
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            ...
            <GlassButton
              variant="passion"
              onClick={() => selectedOperator && handleDeleteOperator(selectedOperator.id)}
              fullWidth
            >
              Confirm Delete
            </GlassButton>
